...
public class Subscribe implements IQuickAction {
	...
	public void execute() throws QCActionException {
		...
		try {
			...
			IDfUser luser = session.getUser(null);
			for (int i = 0; i < idlist.size(); i++) {
				String objid = idlist.get(i);
				IDfSysObject obj = (IDfSysObject) session.getObject(new DfId(objid));
				IDfRelation relation = (IDfRelation) session.getObjectByQualification("dm_relation where relation_name = 'dm_subscription' and parent_id = '" + obj.getString("r_object_id") + "' and child_id = '" + luser.getString("r_object_id") + "'");
				if (relation == null) {
					IDfRelation newrelation = (IDfRelation) session.newObject("dm_relation");
